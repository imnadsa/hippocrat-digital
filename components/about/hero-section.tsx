...
import { ArrowRight, Stethoscope, Brain, TrendingUp } from 'lucide-react';
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="absolute inset-0 opacity-10">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-gradient-to-r from-teal-400 to-indigo-400 rounded-full blur-3xl opacity-20 animate-gradient"></div>
      </div>

      {/* Floating Medical Icons */}
      <div className="absolute inset-0 pointer-events-none">
        <Stethoscope className="absolute top-1/4 left-1/4 w-8 h-8 text-teal-400/30 animate-float" />
        <Brain className="absolute top-3/4 right-1/4 w-10 h-10 text-indigo-400/30 animate-float delay-1000" />
        <TrendingUp className="absolute bottom-1/3 left-1/3 w-6 h-6 text-teal-400/30 animate-float delay-500" />
      </div>
